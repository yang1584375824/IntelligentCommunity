...
2.优化客户端界面，添加物业管理员修改客户端主题功能，重启客户端后生效。

3.优化客户端登录，添加登录界面输入账号密码后可用大小键盘Enter键登录。

4.优化客户端登录界面窗口，取消管理员单独登录窗口，三个身份登录合并1个主登陆界面进行登录，对登录界面提示文字进行优化。

5.优化管理员人事管理模块，将身份标识显示出文字并改用下拉框修改。

6.优化物业业主信息管理模块，增加房产类型信息录入，性别显示改为下拉框显示选择。

7.优化请假界面时间填写方式，用dateTimeEdit代替原有的lineEdit。

8.优化上下班界面，对上下班打卡采取系统自动获取时间并存于数据库中，添加上班迟到和下班早退提示。

9.优化管理员月度出勤界面，添加单独查询和删除出勤记录。

10.优化业主报修地址不再默认业主的当前住址，业主可以手动输入报修地址。

11.修复物业工作人员可以修改业主报修记录业主个人信息和报修地址的bug。

12.优化物业工作人员车位管理模块，增加车位等级录入功能。
